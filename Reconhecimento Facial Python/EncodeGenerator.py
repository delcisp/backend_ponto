    ...
    for userId in os.listdir(folderPath):
        ...
        if os.path.isdir(userPath):
            for imgName in os.listdir(userPath):
                ...
                if encodings:
                    userEncodings.extend(encodings)  # Adiciona todos os encodings encontrados
                else:
                    userImagesWithoutFaces.append(imgName)  # Adiciona o nome da imagem sem rosto detectado
...
# Exibir informações sobre imagens sem rostos detectados
...
# Salvar os encodings em um arquivo
